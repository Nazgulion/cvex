...
- Active report rows show component-scan progress and a percentage, refreshed by the existing five-second polling. Queued/preparing runs start at 0%; exporting shows 100% of components scanned with a separate “writing reports” label. Retried scans reset their progress unless resuming a saved export snapshot.
- Severity counts show green ↑ increases or red ↓ decreases, with the amount changed, against the previous retained published report in the same project (including across SBOM versions). Hover over a change for the baseline date and version. Failed/in-flight runs are skipped, unchanged counts have no arrow, and newly zero counts remain visible when they decreased. The oldest retained report has no baseline. Comparisons involving partial scans are marked as potentially incomplete. These are count differences, not identification of added/resolved CVE IDs.
- Newly generated HTML reports highlight added component/CVE findings with green borders, a tinted background and **NEW FINDING** badges. A top summary identifies the baseline and links to a new finding; affected component groups open automatically and appear first. Matching uses component ID + CVE for the same SBOM, or component name/version + CVE (including duplicate counts) across SBOMs. Severity/description changes alone are not new findings; new and removed findings can coexist with unchanged totals. First reports and unavailable baselines do not label every finding new. Partial baselines are disclosed. Comparison metadata is frozen in the report snapshot/JSON, so retries or deleting the baseline do not alter the saved highlights. Existing saved reports are not rewritten; generate another scan to use this feature. HTML viewing and downloading use the same highlighted artifact; CSV columns remain unchanged.
- “View HTML” opens a stored report in a separate tab. The download icon saves HTML; CSV and JSON links download those formats.
- Admins can choose **Delete report** on an individual row and confirm the project, scan time and SBOM version before permanent deletion. The report's managed files, snapshot and unshared scan/findings are removed; the project, SBOMs, schedule and other reports remain. Queued/running reports cannot be deleted. Failed reports can be removed. Deletion is audited and failed file cleanup retries automatically. Comparison arrows recalculate against the remaining published reports. Pre-adoption archival copies and shared scan data remain intact. No migration is required.
...
- Admins can open a project and choose **Delete project**, then confirm **Delete permanently**. This removes its uploads, versions, schedule, queued runs, all managed reports and their unshared scan/findings data. Running scans/exports block deletion until they finish. The server enforces administrator access and CSRF protection. Other projects, shared SBOM catalog data, source intelligence, audit history and pre-adoption archival copies are preserved. File cleanup is attempted immediately after the database transaction commits; failures are retried by the scheduler's existing cleanup cycle (approximately once a minute). There is no undo. No schema migration is needed for this feature.
...
- **Architecture → NVD/CVE worker → Settings** (or **Settings → NVD/CVE synchronization**) shows a live activity card: last successful sync, latest completed attempt, processed/new-or-updated/unchanged counts, duration, status, and a paginated table covering the last 10 days. It refreshes every five seconds while visible without resetting unsaved settings. Counts measure processed payloads, not necessarily newly published or distinct CVEs. Missing measurements remain `—`. Fetch failures and interrupted attempts are recorded; error messages expose only the exception type, never upstream credentials.
- The activity card lists the next five occurrences in the saved timezone, anchored to the actual saved `next_run` (including retry times). Paused schedules show no upcoming runs. Overdue work is labeled due/waiting; later interval times and forecasts during active syncing are estimates because intervals restart after completion. Cron forecasts follow the existing DST policy. Architecture's compact table shows the latest 12 attempts within the same ten-day window.
- Migration `0005_sync_history` creates a compact, independent operational history and seeds completed NVD/CVE ingestion runs from the last ten days. Seeded rows are labeled **legacy timing**, since older CVE durations may exclude Git preparation. New scheduled-worker rows measure the complete cycle, including fetching. The scheduler prunes completed/interrupted history older than ten days approximately once a minute; source workers also prune at cycle start. Active attempts are preserved and reconciled after worker interruption. Underlying `source_run` provenance, source payloads, SBOMs, and reports are not deleted. Upgrade the schema before restarting the web, scheduler, report worker and both source workers. This additive migration allows rollback to the previous images without a schema downgrade.
